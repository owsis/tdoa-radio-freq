function [ doa_meters, doa_samples, reliability, delay_ref_sample, drift_analysis ] = tdoa2(signal1_complex, signal2_complex, rx_distance_diff, rx_distance, ...
...
%   drift_analysis: 	struct with frequency drift estimates (ppm, confidence, warnings)
%
%   input:
...
if report_level > 2
    ...
    grid;
    % save('hasil_spektrum.mat', 'spectrum1', 'freq_axis');
    % return;
end

%% ======================================================================
%% SNR ANALYSIS (Signal-to-Noise Ratio Detection)
%% ======================================================================
%% Detect low SNR which causes poor correlation reliability

disp(' ');
disp('______________________________________________________________________________________________');
disp('SNR ANALYSIS');

% Estimate SNR for each slice/RX combination
[snr_db_11, ~, ~, ~, qual_11] = estimate_snr_iq(signal11_complex);
[snr_db_12, ~, ~, ~, qual_12] = estimate_snr_iq(signal12_complex);
[snr_db_13, ~, ~, ~, qual_13] = estimate_snr_iq(signal13_complex);

[snr_db_21, ~, ~, ~, qual_21] = estimate_snr_iq(signal21_complex);
[snr_db_22, ~, ~, ~, qual_22] = estimate_snr_iq(signal22_complex);
[snr_db_23, ~, ~, ~, qual_23] = estimate_snr_iq(signal23_complex);

% Display SNR summary
disp(' ');
disp('SNR BY SLICE (in dB):');
disp('─────────────────────');
disp(sprintf('  RX1 Slice1 (Ref):         SNR = %6.2f dB (%s)', snr_db_11, qual_11));
disp(sprintf('  RX1 Slice2 (Measurement): SNR = %6.2f dB (%s)', snr_db_12, qual_12));
disp(sprintf('  RX1 Slice3 (Ref Check):   SNR = %6.2f dB (%s)', snr_db_13, qual_13));
disp(' ');
disp(sprintf('  RX2 Slice1 (Ref):         SNR = %6.2f dB (%s)', snr_db_21, qual_21));
disp(sprintf('  RX2 Slice2 (Measurement): SNR = %6.2f dB (%s)', snr_db_22, qual_22));
disp(sprintf('  RX2 Slice3 (Ref Check):   SNR = %6.2f dB (%s)', snr_db_23, qual_23));

% Calculate average SNR per slice/band
avg_snr_slice1 = mean([snr_db_11, snr_db_21]);
avg_snr_slice2 = mean([snr_db_12, snr_db_22]);
avg_snr_slice3 = mean([snr_db_13, snr_db_23]);

disp(' ');
disp('AVERAGE SNR BY FREQUENCY BAND:');
disp('───────────────────────────────');
disp(sprintf('  Band 1 (Reference):   %6.2f dB', avg_snr_slice1));
disp(sprintf('  Band 2 (Measurement): %6.2f dB', avg_snr_slice2));
disp(sprintf('  Band 3 (Ref Check):   %6.2f dB', avg_snr_slice3));

% Identify problematic slices
snr_threshold_good = 15;  % dB
snr_threshold_poor = 5;   % dB

if snr_db_12 < snr_threshold_good || snr_db_22 < snr_threshold_good || ...
        (snr_db_13 < snr_threshold_good && snr_db_13 < snr_db_11 - 2) || ...
        (snr_db_23 < snr_threshold_good && snr_db_23 < snr_db_21 - 2)
    
    disp(' ');
    disp('⚠️  LOW SNR DETECTED:');
    disp('─────────────────────');
    
    if snr_db_12 < snr_threshold_good
        disp(sprintf('    • RX1 Slice2 (Measurement): %.1f dB', snr_db_12));
    end
    if snr_db_22 < snr_threshold_good
        disp(sprintf('    • RX2 Slice2 (Measurement): %.1f dB', snr_db_22));
    end
    if snr_db_13 < snr_threshold_good && snr_db_13 < snr_db_11 - 2
        disp(sprintf('    • RX1 Slice3 (Ref Check): %.1f dB (degraded)', snr_db_13));
    end
    if snr_db_23 < snr_threshold_good && snr_db_23 < snr_db_21 - 2
        disp(sprintf('    • RX2 Slice3 (Ref Check): %.1f dB (degraded)', snr_db_23));
    end
    
    if avg_snr_slice2 < snr_threshold_poor
        disp(' ');
        disp('*** CRITICAL: Measurement signal (Band 2) has VERY LOW SNR! ***');
        disp('    This DIRECTLY explains low correlation reliability!');
        disp('    Root causes to investigate:');
        disp('    • Interference from adjacent/nearby FM station (use spectrum analyzer)');
        disp('    • Weak TX signal at this frequency (check transmitter power)');
        disp('    • Poor antenna impedance match at Band 2 frequency');
        disp('    • High ambient noise floor environment');
        disp('    ');
    elseif avg_snr_slice2 < snr_threshold_good
        disp(' ');
        disp('*** WARNING: Measurement signal (Band 2) SNR is marginal (<15 dB). ***');
        disp('    This will reduce correlation peak sharpness and reliability.');
    end
    
    if avg_snr_slice3 < avg_snr_slice1 - 2
        disp(' ');
        disp('*** INFO: Band 3 SNR degraded compared to Band 1 (~3+ dB difference). ***');
        disp('    Possible cause: RTL-SDR tuner heating after 15-30 minutes of operation.');
        disp('    Solution: Allow cooling time between recordings, or use tuner temperature');
        disp('             compensation if available in your RTL-SDR driver.');
    end
end
...
disp(['delay1_native: ', num2str(delay1_native)]);
disp(['delay1_native: ', num2str(length(signal11_complex))]);
disp(['delay1_native: ', num2str(corr_signal_1(idx1))]);
...
%%  zur Sicherheit = untuk antisipasi
...
disp(['rx_distance_diff : ', num2str(rx_distance_diff )]);
% disp(['valid_samples_right: ', num2str(valid_samples_right)]);
% disp(['valid_samples_right: ', num2str(valid_samples_right+1+2)]);

for ii=1:valid_samples_right+1+2   % +1 for safety if rounding down and +2 for possible frequency drift
    delay_mask(idx1+ii) = 0.8;
end

for ii=1:valid_samples_left+1+2   % +1 for safety if rounding down and +2 for possible frequency drift
    delay_mask(idx1-ii) = 0.8;
end
...
% disp(['idx1: ', num2str(idx1)]);
% disp(['idx1: ', num2str(length(corr_signal_2))]);
% disp(['idx1: ', num2str(corr_signal_2_valid(idx1))]);
...
% corr_signal_2_valid = corr_signal_2; % truncation switch off 99.8554% hma 71.6603% hba

corr2_reliability = corr_reliability(corr_signal_2_valid);

[~, idx2] = max(corr_signal_2_valid);
% disp(['idx2: ', num2str(idx2)]);
delay2_native = idx2 - length(signal12_complex); % >0: signal1 later, <0 signal2 later
disp(['delay2_native: ', num2str(delay2_native)]);
disp(['delay2_native: ', num2str(length(signal12_complex))]);
disp(['delay2_native: ', num2str(corr_signal_2_valid(idx2))]);
...
if (interpol > 1)
    ...
    for ii=1:interpol*(valid_samples_right+1+2)   % +1 for safety if rounding down and +2 for possible frequency drift
        corr_signal_2_valid_interp(idx1_interp_i+ii) = corr_signal_2_interp(idx1_interp_i+ii);
    end
    
    for ii=1:interpol*(valid_samples_left+1+2)   % +1 for safety if rounding down and +2 for possible frequency drift
        corr_signal_2_valid_interp(idx1_interp_i-ii) = corr_signal_2_interp(idx1_interp_i-ii);
    end
...
disp(['delay3_native: ', num2str(delay3_native)]);
disp(['delay3_native: ', num2str(length(signal13_complex))]);
disp(['delay3_native: ', num2str(corr_signal_3(idx3))]);
...
if (report_level > 0)
    ...
    if (interpol <= 1)
        plot(idx_left:idx_right, corr_signal_2(idx_left:idx_right),'o-',...
            idx_left:idx_right, delay_mask(idx_left:idx_right), 'r', idx2, corr_signal_2(idx2),'dr');
        legend('Sinyal Korelasi', 'Delay Mask', 'Puncak Terdeteksi', 'Location', 'best');
        text(idx2, corr_signal_2(idx2), sprintf(' Idx: %d\n Nilai: %.3f', idx2, corr_signal_2(idx2)), ...
            'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'FontSize', 9, 'Color', 'r', 'FontWeight', 'bold');
    else
        plot(idx_left:idx_right, corr_signal_2(idx_left:idx_right),'o-',...
            x_corr_interp_plot((interpol*idx_left):(interpol*idx_right)), corr_signal_2_interp((interpol*idx_left):(interpol*idx_right)), 'm.',...
            idx_left:idx_right, delay_mask(idx_left:idx_right), 'r', idx2_interp, corr_signal_2_interp(idx2_interp_i),'dr');
        legend('Sinyal Korelasi Asli', 'Data Interpolasi', 'Delay Mask', 'Puncak Terdeteksi', 'Location', 'best');
        text(idx2_interp, corr_signal_2_interp(idx2_interp_i), sprintf(' Idx: %.2f\n Nilai: %.3f', idx2_interp, corr_signal_2_interp(idx2_interp_i)), ...
            'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'FontSize', 9, 'Color', 'r', 'FontWeight', 'bold');
    end
...
if abs(delay1 - delay3) <= 2 % cek kondisi delay1 dan delay3, idealnya delay sample tidak boleh lebih dari 2
    ...
    if corr1_reliability > corr3_reliability % pilih avg_delay13 berdasarkan reliabilitas yang lebih tinggi
        avg_delay13 = delay1;
...
% disp(['rx_dist => ', num2str(rx_distance_diff)]);
% disp(['samp_rate => ', num2str(sample_rate)]);
ref_signal_diff_samples = (rx_distance_diff / 3e8) * sample_rate; % known ref signal delay in samples

% disp(['avg_delay13 => ', num2str(avg_delay13)]); % 47042.5
% disp(['reff_sign => ', num2str(ref_signal_diff_samples)]); % -29.6064
...
% doa_samples = delay2; % 27.0227
% doa_meters = (doa_samples / (rx_distance / (3e8 / sample_rate))) * rx_distance;
% akurasi_hyperbola = 71.6603%
% akurasi_heatmap = 99.8554%
% akurasi_hyperbola = 88.8565% (dengan delay1 dan delay3 / 2)
% akurasi_heatmap = 86.8754% (dengan delay1 dan delay3 / 2)

%% ======================================================================
%% FREQUENCY DRIFT ANALYSIS
%% ======================================================================
%% Detect clock frequency drift from timing inconsistencies
%% between reference signals measured at different times.

disp(' ');
disp('______________________________________________________________________________________________');
disp('FREQUENCY DRIFT ANALYSIS');

% Detect drift for RX pair 1-2
[drift_ppm_12, drift_conf_12, drift_warn_12] = detect_frequency_drift(...
    delay1_native, delay2_native, delay3_native, sample_rate, 'RX1-RX2');

% Detect drift for RX pair 1-3
[drift_ppm_13, drift_conf_13, drift_warn_13] = detect_frequency_drift(...
    delay1_native, delay2_native, delay3_native, sample_rate, 'RX1-RX3');

% Detect drift for RX pair 2-3
[drift_ppm_23, drift_conf_23, drift_warn_23] = detect_frequency_drift(...
    delay1_native, delay2_native, delay3_native, sample_rate, 'RX2-RX3');

% Package results into structure
drift_analysis.ppm_12 = drift_ppm_12;
drift_analysis.ppm_13 = drift_ppm_13;
drift_analysis.ppm_23 = drift_ppm_23;
drift_analysis.conf_12 = drift_conf_12;
drift_analysis.conf_13 = drift_conf_13;
drift_analysis.conf_23 = drift_conf_23;
drift_analysis.warn_12 = drift_warn_12;
drift_analysis.warn_13 = drift_warn_13;
drift_analysis.warn_23 = drift_warn_23;
drift_analysis.delay_diff_13 = delay1_native - delay3_native;  % Ground truth timing consistency

% Print summary
disp(' ');
disp('DRIFT DETECTION SUMMARY');
disp(['  RX1-RX2 drift: ' num2str(drift_ppm_12, '%+7.2f') ' ppm, confidence: ' num2str(drift_conf_12, '%.2f') ', warning: ' int2str(drift_warn_12)]);
disp(['  RX1-RX3 drift: ' num2str(drift_ppm_13, '%+7.2f') ' ppm, confidence: ' num2str(drift_conf_13, '%.2f') ', warning: ' int2str(drift_warn_13)]);
disp(['  RX2-RX3 drift: ' num2str(drift_ppm_23, '%+7.2f') ' ppm, confidence: ' num2str(drift_conf_23, '%.2f') ', warning: ' int2str(drift_warn_23)]);

% Calculate average drift magnitude
avg_drift_magnitude = mean([abs(drift_ppm_12), abs(drift_ppm_13), abs(drift_ppm_23)]);
disp(['  Average absolute drift: ' num2str(avg_drift_magnitude, '%.2f') ' ppm']);

% Warn if significant drift detected
if avg_drift_magnitude > 50
    disp('');
    disp('*** CRITICAL WARNING: Significant frequency drift detected! ***');
    disp('    This explains low correlation reliability and poor TDOA accuracy.');
    disp('    SOLUTION: Use external reference clock or GPS-PPS disciplined oscillator.');
elseif avg_drift_magnitude > 10
    disp('');
    disp('*** WARNING: Moderate frequency drift detected (>10 ppm). ***');
    disp('    Consider external clock synchronization for next measurement.');
end

disp(' ');

reliability = min([corr3_reliability, corr2_reliability, corr1_reliability]);
delay_ref_sample = abs(delay1 - delay3);
